    ...
    def validate_test_relation(passage, question):
        ...
        if passage.test_id != question.test_id:
            raise serializers.ValidationError(
                {'detail': 'Прохождение и вопрос должны '
                           'относиться к одному тесту'}
            )

    @staticmethod
    def validate_passage_user(passage, request):
        """Проверяет, что текущий пользователь является инициатором прохождения
         и может создавать к нему ответы"""
        if request.method == 'POST' and passage.user_id != request.user.id:
            raise serializers.ValidationError(
                {'detail': 'Текущий пользователь не является '
                           'инициатором прохождения'}
            )
